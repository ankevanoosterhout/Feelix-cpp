...
// std::string Feelix_Mode::effect_data_transmit;
bool Feelix_Mode::active;
// std::string SerialListen::serial_data_str_tx;
...
//     xTaskCreate(feelix_monitor, "feelix_monitor", 2048, NULL, 10, NULL);
// 	xTaskCreate(tx_task, "tx_task", 2048, NULL, 10, NULL);

// } 

// void SerialListen::tx_task(void *arg)
// {
// 	// int num = 0;
//     while (1) {
//     	// sprintf ("hello");
//         uart_write_bytes(0, serial_data_str_tx.data(), serial_data_str_tx.size() * sizeof(char));
...
// 		strcpy(knob->effect_file, serial_data_str.data());
// 		serial_data_str_tx.assign(serial_data_str.data());
...
//     }
// }
// void SerialListen::feelix_monitor(void *pvParameters) {
// 	while(1){
// 		serial_data_length = uart_read_bytes(0, serial_data, SERIAL_BUFSIZE, 20 / portTICK_RATE_MS);
//         if (serial_data_length > 0) {
//             serial_data_str = convertToString(serial_data);
...
Feelix_Mode::Feelix_Mode(): Mode(MAX_TORQUE) {
}

void Feelix_Mode::set_active(bool is_active) {
	active = is_active;
}

int16_t Feelix_Mode::calc(void* ptr){
	TorqueTuner* knob = (TorqueTuner*)ptr;
	// if(knob->effect_file[0] != NULL){
	// 	effect_data_transmit = convertToString(knob->effect_file);
	// }
	return knob->angle;
}
