...
//     static std::string serial_data_str_tx;
...
//     static void feelix_monitor(void *pvParameters);
//     static void tx_task(void *arg);
// };

class Feelix_Mode: public Mode
{
public:
    Feelix_Mode();
    int16_t calc(void* ptr);
    static std::string effect_data_transmit;
    void set_active(bool is_active);
private:
    static bool active;
    static inline std::string convertToString(char* a) {
...
public:
    ...
    // SerialListen serial;
    Feelix_Mode feelix;
    std::vector<Mode * > mode_list = {&click, &magnet, &wall, &inertia, &lin_spring, &exp_spring,  &free, &spin, &feelix};
    Mode * active_mode;
